...
async def start_assistant():
    ...
    if not s:
        return None
    user = Client("user_assistant", session_string=s, api_id=API_ID, api_hash=API_HASH)
    await user.start()
    vc = PyTgCalls(user)
    await vc.start()
    return user
...
async def start_cmd(_, m):
    await m.reply_text(
        "🎧 *VC Music Bot Ready*\n"
        f"👑 Owner: @{OWNER}\n\n"
        "Send: `/string <session>` to set assistant session.\n"
        "Then use /play in groups."
    )

@bot.on_message(filters.private & filters.command("string"))
async def session_cmd(_, m):
    if m.from_user.username != OWNER:
        return await m.reply("❌ Only owner allowed.")
    if len(m.text.split()) < 2:
        return await m.reply("Use: `/string SESSION_STRING`")
    save_session(m.text.split(None, 1)[1])
    await m.reply("✅ Session saved!")
    if await start_assistant():
        await m.reply("Assistant started!")
...
async def play_cmd(_, m):
    ...
    await m.reply_photo(
        info["thumb"],
        caption=f"▶️ **Playing:** {info['title']}\n👤 Requested by: @{m.from_user.username}",
